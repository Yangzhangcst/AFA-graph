...
para.epochs = 20;
para.d = 50;
para.Nimgs = 300; % number of images in BSDS300/500

%% read numbers of segments used in the paper
bsdsRoot ='./database/BSDS300';
fid = fopen(fullfile('Nsegs_300.txt'),'r');
...
for idxI =1:para.Nimgs
    ...
    out_path = fullfile('results_searchN_denoise_n','BSDS300','ikde_1_1',img_name);
...
%         p = length(global_nodes);
    ...
    %% Transfer cut
    out_path_gt= fullfile('results_searchN_denoise_n','BSDS300','ikde_1_1',img_name);
    ...
    out_seg_path = fullfile('results_searchN_denoise_n','BSDS300','ikde_1_1','segs');
...
fid_out = fopen(fullfile('results_searchN_denoise_n','BSDS300','ikde_1_1','evaluation.txt'),'w');
for idxI=1:para.Nimgs
...
fid_out2 = fopen(fullfile('results_searchN_denoise_n','BSDS300','ikde_1_1','Nsegs.txt'),'w');
for idxI=1:para.Nimgs
