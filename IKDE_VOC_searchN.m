...
para.epochs = 20;
para.d = 50;
para.Nimgs = 1449; % number of images in BSDS300/500

%% read numbers of segments used in the paper
bsdsRoot = './database/VOC/';
saveRoot = 'results_searchN_denoise_n';
fid = fopen(fullfile('Nsegs_VOC.txt'),'r');
line = 1;
while feof(fid) == 0    
...
for idxI = 1:para.Nimgs
    ...
    out_path = fullfile(saveRoot,'VOC','ikde_1_1_APC_3_5',img_name);
    ...
    %% Transfer cut
    out_path_gt= fullfile(saveRoot,'VOC','ikde_1_1_APC_3_5',img_name);
    ...
    out_seg_path = fullfile(saveRoot,'VOC','ikde_1_1_APC_3_5','segs');
...
fid_out = fopen(fullfile(saveRoot,'VOC','ikde_1_1_APC_3_5','evaluation.txt'),'w');
for idxI=1:para.Nimgs
...
fid_out2 = fopen(fullfile(saveRoot,'VOC','ikde_1_1_APC_3_5','Nsegs.txt'),'w');
for idxI=1:para.Nimgs
